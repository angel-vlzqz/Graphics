...
void rotate90CW(unsigned char* image, int width, int height, int channels) {
    ...
    memcpy(image, rotated, width * height * channels);
    delete[] rotated;
}

void mirrorVertical(unsigned char* image, int width, int height, int channels) {
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width / 2; x++) {
            for (int c = 0; c < channels; c++) {
                int leftIndex = (y * width + x) * channels + c;
                int rightIndex = (y * width + (width - x - 1)) * channels + c;
                std::swap(image[leftIndex], image[rightIndex]);
            }
        }
    }
}
...
int main() {
    ...
    GLuint cubeTextures[7];
    glGenTextures(7, cubeTextures);

    
    GLuint sphereTexture;

    glGenTextures(1, &sphereTexture);
    glBindTexture(GL_TEXTURE_2D, sphereTexture); // All upcoming GL_TEXTURE_2D operations now have effect on our texture object
    ...
    unsigned char* sphereImage = SOIL_load_image("Images/Bump-Picture.jpg", &sphereWidth, &sphereHeight, 0, SOIL_LOAD_RGB);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, sphereWidth, sphereHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, sphereImage);
    ...
    glGenTextures(1, &cylTexture);
    glBindTexture(GL_TEXTURE_2D, cylTexture); // All upcoming GL_TEXTURE_2D operations now have effect on our texture object
    // Set our texture parameters
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);	// Set texture wrapping to GL_REPEAT
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    // Set texture filtering
    ...
    int cylWidth, cylHeight;
    unsigned char* clinderImage = SOIL_load_image("Images/Bump-Map.jpg", &cylWidth, &cylHeight, 0, SOIL_LOAD_RGB);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, cylWidth, cylHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, clinderImage);
    glGenerateMipmap(GL_TEXTURE_2D);
    SOIL_free_image_data(clinderImage);
    glBindTexture(GL_TEXTURE_2D, 0); // Unbind texture when done, so we won't accidentily mess up our texture.



    const char* cubeTexture_files[6] = {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        unsigned char* image = SOIL_load_image(cubeTexture_files[i], &width, &height, &channels, SOIL_LOAD_RGB);
        if (!image) {
            std::cout << "Failed to load texture: " << cubeTexture_files[i] << std::endl;
            continue;
        }

        if (i != 0 && i != 5 && i != 3)
            mirrorVertical(image, width, height, channels);
        
        // Rotate the 180 degrees
        if (i == 4 || i==5 || i == 3) { // Rotate specific images as needed
            rotate90CW(image, width, height, channels);
            rotate90CW(image, width, height, channels);
        }
        if (i == 0 || i == 1) { // Rotate specific images as needed
            rotate90CW(image, width, height, channels);
        }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Pass to shaders
        glUniform3f(cubeColorLoc, 1.0f, 1.0f, 1.0f); // Pass cube color to uniform
        ...
        { 
            ...
            glBindTexture(GL_TEXTURE_2D, cubeTextures[i]); 
            glUniform1i(glGetUniformLocation(cubeShader.Program, "texture1"), 0); // Set the sampler to use texture unit 0
            if (i == 0) { 
                // Negative X (Left) 
        ...
        glUniform3f(cylinderColorLoc, 1.0f, 1.0f, 1.0f); // Pass color to uniform
        glUniform3f(lightColorLoc, 1.0f, 1.0f, 1.0f); // Pass light color to uniform
        glUniform3f(lightPosLoc, lightPos.x, lightPos.y, lightPos.z); // Pass light position to uniform
        glUniform3f(viewPosLoc, camera.Position.x, camera.Position.y, camera.Position.z); // Pass camera position to uniform
        ...
        view_cylinder = glm::translate(view_cylinder, glm::vec3(-1.7f, -2.95f, -5.0f)); // Translate cylinder back, to the right, and down
        ...
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, cylTexture);
        glUniform1i(glGetUniformLocation(cylinderShader.Program, "texture1"), 0);
        ...
        glUniform3f(sphereColorLoc, 1.0f, 1.0f, 1.0f); // Pass in sphere color to uniform
        ...
        glUniform1i(glGetUniformLocation(sphereShader.Program, "texture1"), 0);
